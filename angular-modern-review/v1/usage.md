...
```typescript
// ❌ MAL - Depender del app.module.ts gigante

// ✅ BIEN - Componentes independientes (Angular 14+)
@Component({
  selector: 'app-user',
  standalone: true,
...
// ✅ BIEN - Función inject() (Más composable y sin constructores largos)
...
// ✅ BIEN - Usamos el contexto de inyección para limpiarlo automáticamente
constructor() {
  this.http.get('/api/users').pipe(
    takeUntilDestroyed() // 🚀 Adiós a fugas de memoria
  ).subscribe();
}
```
...
```typescript
// ❌ MAL - Todo suelto con `any` o variables flojas fallará en Angular 14+ strict mode
const form = new FormGroup({
  name: new FormControl() 
});

// ✅ BIEN - Formularios fuertemente tipados
const form = new FormGroup({
...
```typescript
// ✅ BIEN - Usar Signals para variables locales sin RxJS excesivo
users = signal<User[]>([]);
