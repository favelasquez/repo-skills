...
No module registration needed — standalone component model:
...
onGridReady(event: GridReadyEvent<Product>): void {
  // Single unified API — no separate columnApi
...
## Notes
- Requires Angular 17+
- Legacy `columnApi` is removed — all methods are on `gridApi`
