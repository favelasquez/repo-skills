...
Señales en el código:
  ...
  $guarded = []                          �  �a�️ sin protección de mass assignment
...
1. �x� Seguridad � Mass Assignment, SQL Injection, Auth/Policies, IDOR
2. �x� Eloquent � N+1, eager loading, scopes mal usados
3. �xx� Arquitectura � lógica en controladores, Service Layer, Repository
4. �xx� Performance � caché, queries lentas, jobs/queues, índices
...
```php
// �R N+1 � por cada post se ejecuta una query para cargar el autor
$posts = Post::all(); // SELECT * FROM posts
foreach ($posts as $post) {
    echo $post->author->name; // SELECT * FROM users WHERE id = ? (x N veces)
}

// �S& Eager loading � un solo JOIN
$posts = Post::with('author')->get();
foreach ($posts as $post) {
    echo $post->author->name; // ya cargado, 0 queries extra
}

// �S& Lazy eager loading � cuando no sabes si necesitarás la relación
$posts = Post::all();
if ($needsAuthors) {
    $posts->load('author'); // una sola query adicional para todos
}

// �S& Con select � solo las columnas necesarias
...
```php
// �R $guarded vacío � cualquier campo del request se asigna a la DB
...
// �S& CORRECTO � lista explícita de campos permitidos
class User extends Model
{
...
// �S& En el controlador � validar Y limitar campos
...
```php
// �R Cualquier usuario autenticado puede ver/editar órdenes ajenas
public function show(Order $order)
{
    return new OrderResource($order); // no verifica que sea del usuario actual
}

// �S& CORRECTO � Opción A: verificar manualmente
public function show(Order $order)
{
...
// �S& CORRECTO � Opción B: Policy (recomendada, reutilizable)
...
// �S& CORRECTO � Opción C: scopear la query al usuario (más segura)
...
```php
// �R Controlador gordo � validación, lógica, envío de emails, todo mezclado
...
// �S& Controlador delgado � delega a Service + Form Request
...
```php
// �R Expone todos los campos, incluyendo password, tokens, campos internos
public function index()
{
    return User::all(); // JSON con password_hash, remember_token, etc.
}

// �S& API Resource � contrato explícito de lo que se expone
...
DB::transaction(function () use ($userId, $productId) {
    ...
    $user->decrement('credits', 10);  // A: credits = 0 | B: credits = -10 �x�
});

// �S& CON lockForUpdate() � Bloqueo pesimista
...
DB::transaction(function () use ($userId, $productId) {
    ...
    // COMMIT �  libera el lock �  el proceso B lee credits = 0 �  excepción �S&
});
```
...
- [ ] ¿Las validaciones de negocio (saldo suficiente, stock > 0) ocurren DESPU�0S del lock?
