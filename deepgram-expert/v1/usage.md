...
```python
# �R Deepgram asume PCM 16kHz � recibe μ-law 8kHz �  transcripción basura
dg_connection = await client.transcription.live({
    'punctuate': True,
})

# �S& SIEMPRE especificar encoding y sample_rate para Twilio
dg_connection = await client.transcription.live({
    'encoding':    'mulaw',
    'sample_rate': 8000,
    'channels':    1,
...
```python
# �R Con interim_results=True, Deepgram envía actualización por cada palabra
...
```python
# �R Deepgram cierra la conexión si no recibe audio por ~10-12 segundos
# Pausas largas, música en espera, usuario pensando �  transcripción se corta

# �S& OPCI�N A � enviar silencio μ-law periódicamente
...
# �S& OPCI�N B � KeepAlive message (SDK v3)
...
```python
# �R Solo con is_final no se sabe cuándo el usuario TERMIN� su turno
# (vs una pausa breve dentro de una frase larga)

# �S& Usar utterance_end_ms + evento UtteranceEnd (SDK v3)
options = LiveOptions(
    encoding="mulaw",
    sample_rate=8000,
    model="nova-2-phonecall",
...
- �x� Agente no para cuando el usuario habla encima (**barge-in ausente**)
- �x� Buzón de voz no se detecta o el agente trata de hablar con la máquina
- �x� Colgar no es limpio � la llamada queda abierta o cuelga abruptamente
- �x� Agente no detecta cuándo el usuario terminó su turno
- �x� Lag alto entre que el usuario habla y el agente responde
