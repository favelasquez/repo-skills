...
Tu servidor
    │
    ├─ 1. calls.create(to, from, url, machine_detection) ──→ Twilio REST API
    │                                                              │
...
```python
# ❌ Twilio recibe HTML/JSON en lugar de XML → cuelga la llamada
@app.post("/webhook")
async def webhook():
    return {"twiml": "<Response><Say>Hola</Say></Response>"}  # JSON — incorrecto

# ✅ Siempre application/xml
...
```xml
<!-- ❌ Twilio ejecuta el Say y cuelga — sin tiempo para conversación -->
<Response>
...
```python
# ❌ Solo cerrar el WebSocket — Twilio puede tardar 5s en detectarlo
await websocket.close()

# ❌ Responder TwiML con Hangup — solo funciona en respuesta directa al webhook
# No funciona para colgar desde el servidor en cualquier momento

# ✅ Llamar a la REST API — instantáneo
...
```python
# ❌ El evento 'start' aún no llegó — stream_sid desconocido
...
# ✅ Capturar stream_sid en el evento 'start' antes de enviar audio
stream_sid = None
...
```python
# ❌ Cancelar el TTS pero el audio ya enviado sigue sonando en Twilio
tts_task.cancel()  # cancela la generación, pero Twilio ya tiene audio en buffer

# ✅ Enviar 'clear' + cancelar el task
...
```python
# ❌ Sin validación — cualquiera puede enviar requests a tu webhook
@app.post("/webhook")
