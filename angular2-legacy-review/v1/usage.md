...
```typescript
// �S& CORRECTO � Siempre implementar la interfaz
export class MiComponente implements OnInit, OnDestroy {
  ngOnInit(): void { }
  ngOnDestroy(): void { }
}

// �R MAL � Sin interfaz implementada
...
```typescript
// �S& Angular 2 � Inyectar en constructor
constructor(
...
// �R MAL en Angular 2 � inject() no existe aún
...
export class EjemploComponent implements OnInit, OnDestroy {
  ...
  // �S& Array tipado de suscripciones
  private subscriptions: Subscription[] = [];

  ngOnInit(): void {
    // �S& Agregar cada suscripción al array
  ...
  ngOnDestroy(): void {
    // �S& Matar TODAS las suscripciones en un bucle
...
```typescript
// �R MAL � EL PATR�N `takeUntil()` o `takeUntilDestroyed()` NO ESTÁ DISPONIBLE EN ESTE STACK
...
// �xx� SOLUCI�N: Usar SIEMPRE el Patrón 1 (Array de suscripciones) o el Patrón 2 (Subscription.add()),
...
```typescript
// �S& CORRECTO � Interfaces en archivos separados (models/)
...
```typescript
// �S& Angular 2.4.x usa Http, NO HttpClient (ese es Angular 4.3+)
...
```typescript
// �S& Importar en NgModule
import { AgGridModule } from 'ag-grid-angular';
// En imports: AgGridModule.withComponents([])

// �S& Componente con ag-grid 18
...
> �a�️ **ag-grid 18 vs versiones modernas**: En v18, `sortable`, `resizable` y `filter` se configuran en `gridOptions`, NO en cada `ColDef`. Esto cambió en v20+.
...
```typescript
// �S& RxJS 5 � importar operadores individualmente (NO importar todo)
import { Observable } from 'rxjs/Observable';
...
// �R MAL � importación moderna (RxJS 6+, no existe en RxJS 5)
